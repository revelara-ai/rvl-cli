//! `init` ported from rvl-cli `internal/commands/init.go`:
...
/// The `.revelara/memory/digest.compact` skeleton (a tracked follow-up in rvl-cli).
...
fn build_project_config(
    ...
    if yes {
        // a tracked follow-up/.23: unattended runs declare NOTHING - detections
    ...
    if crate::confirm("Declare these components as services?") {
        // a tracked follow-up: explicit acceptance IS the act of declaration.
...
fn render_config_yaml(project: &str, declared: &[Component], candidates: &[Component]) -> String {
    ...
    // a tracked follow-up: declared components (an explicit human act - interactive
    ...
    //: each uncommented entry becomes a first-class
...
/// a tracked follow-up: a workspace member is a service CANDIDATE only when it is
...
mod tests {
    ...
    fn rust_workspace_library_members_are_not_candidates() {
        // a tracked follow-up: a workspace member without a binary target is an
    ...
    fn declared_components_render_active_candidates_commented() {
        // a tracked follow-up: interactive acceptance IS a declaration - declared
    ...
    fn init_candidates_commented() {
        // a tracked follow-up: detected components are CANDIDATES. They are
    ...
    fn detects_go_work_and_package_json_workspaces() {
        ...
        .unwrap();
        // a tracked follow-up: go.work members pass the gate only when runnable.

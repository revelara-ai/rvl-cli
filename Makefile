...
## build: compile the rvl workspace binaries
build:
	$(CARGO) build $(CARGO_FLAGS)

## rebuild: force a clean recompile of the rvl binary
rebuild:
	$(CARGO) build $(CARGO_FLAGS) -p rvl
...
install:
	...
	@if command -v node >/dev/null 2>&1; then \
	  ...
	  echo "installed rvl + goindex + pyindex.py + tsindex.js + javaindex.java to $(BINDIR)"; \
	  echo "  Go, Python, and TypeScript scanning work with zero env; Java needs a JDK 11+ on PATH."; \
	else \
	  echo "installed rvl + goindex + pyindex.py + javaindex.java to $(BINDIR)"; \
	  echo "  Go + Python work out of the box; TypeScript needs Node; Java needs a JDK 11+."; \
	fi
	@case ":$$PATH:" in *":$(BINDIR):"*) : ;; *) echo "  NOTE: $(BINDIR) is not on your PATH — add it, then: rvl scan <path>";; esac

## uninstall: remove the installed binary + helpers from BINDIR
uninstall:
	rm -f "$(BINDIR)/rvl" "$(BINDIR)/goindex" "$(BINDIR)/cindex" "$(BINDIR)/rustindex" "$(BINDIR)/pyindex.py" "$(BINDIR)/tsindex.js" "$(BINDIR)/javaindex.java"
...
helpers: build
	...
	@echo "helpers installed next to $(BIN_DIR)/rvl (goindex, pyindex.py, javaindex.java, cindex, rustindex)"
...
dev: helpers
	@echo "run: $(BIN_DIR)/rvl scan <path>"
